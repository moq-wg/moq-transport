...
The MediSession ID to given to each subscriber via an out-of-band mechanism. The subscriber uses this MediaSessionID to request the CATALOG, which in turn provides information to the client about the MediaStreamD and MediaGroup IDs which it can use to construct its subscription requests. 
