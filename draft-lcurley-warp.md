...
The basic element of Warp is *a media object*. A media object is a single addressable cacheable unit that may either contain a sequence of media samples, or some media-specific metadata, and may have relay-related attributes such as TTL or delivery priority associated with it. Typically a media object is a sequence of bytes, however, in some formats, it may be a sequence of datagrams. A Warp media object is similar in function to what is often referred to as "segments" or "chunks" in other media protocols; however, they are different from the traditional notion of chunks. The first key distinction is that Warp media objects are not always expected to be fully available, and thus any relays have to be able to convey partial media chunks. The second key distinction is that Warp media objects may not be fully decodable by themselves; an object will contain a description of the prerequisites if that is the case.

*A media feed* in Warp is a combination of *an init object* and a sequence of media objects. An init object is a format-specific self-contained description of the feed that is required to decode any media object contained within the feed, but can also be used as the metadata for feed selection. If two media feeds carry semantically equivalent but differently encoded media, they are referred to as *variants* of each other.

*A Warp broadcast* is a collection of multiple media feeds produced by a single origin. When subscribing to a broadcast, a peer has an option of subscribing to one, many or all media feeds within the broadcast.

A Warp broadcast is globally identifiable via a URI. Within the broadcast, every media feed is identified via *a feed name*, an application-specific binary string. Within a single media feed, every media object is identified by its *sequence number*.

Depending on the profile of the application using it, Warp supports both a mode of operation where the peer unilaterally sends a broadcast with the media feeds of its choice, and a mode where the peer has to explicitly subscribe to a broadcast and select media feeds it wishes to receive. In the latter case, the peer requesting a subscription provides a URI identifying the broadcast, as well as the list of the names of feeds it wishes to subscribe to; if the names of the feeds are not known to the client in advance, it may request the list of all feeds available within the broadcast (which is known as *the broadcast description*).

As an example, consider a scenario where `example.org` hosts a simple live stream that anyone can subscribe to. That live stream would be a single Warp broadcast identified by the URL `https://example.org/livestream`. In the simplest implementation, it would provide only two media feeds with names `audio` and `video`. In more complicated scenarios, it could provide multiple video formats, with names like `360p_h264` or `1080p_av1`; those feeds would be *variants* of each other. Note that the feed names are opaque on the Warp level; if the player is not aware of the naming scheme for the media feeds in advance, it would have to request the broadcast description first.
