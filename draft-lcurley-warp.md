...
A Warp broadcast is globally identifiable via a URI. Within the broadcast, every media track is identified via *a track name*, an application-specific binary string. Within a single media track, every media object is identified by its *sequence number*.

Depending on the profile of the application using it, Warp supports both a mode of operation where the peer unilaterally sends a broadcast with the media tracks of its choice, and a mode where the peer has to explicitly subscribe to a broadcast and select media tracks it wishes to receive. In the latter case, the peer requesting a subscription provides a URI identifying the broadcast, as well as the list of the names of tracks it wishes to subscribe to; if the names of the tracks are not known to the client in advance, it may request the list of all tracks available within the broadcast (which is known as *the broadcast description*).

As an example, consider a scenario where `example.org` hosts a simple live stream that anyone can subscribe to. That live stream would be a single Warp broadcast identified by the URL `https://example.org/livestream`. In the simplest implementation, it would provide only two media tracks with names `audio` and `video`. In more complicated scenarios, it could provide multiple video formats, with names like `360p_h264` or `1080p_av1`; those tracks would be *variants* of each other. Note that the track names are opaque on the Warp level; if the player is not aware of the naming scheme for the media tracks in advance, it would have to request the broadcast description first.
