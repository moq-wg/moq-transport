...
A Warp broadcast is globally identifiable via a URI. Within the broadcast, every media track is identified via *a track ID*, an unsigned 62-bit integer unique within the broadcast. Within a single media track, every media object is identified by an *object ID*, an unsigned 62-bit integer unique within the track.
