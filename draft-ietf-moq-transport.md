...
sending a SUBSCRIBE_UPDATE (TODO: SUBSCRIPTION_UPDATE). If the receiver of the
parameter has one or more updated AUTHORIZATION_TOKENs, it SHOULD include those
in the SUBSCRIBE_UPDATE. Relays that send this parameter and applications that
receive it MAY introduce jitter to prevent many endpoints from updating
simultaneously. This value is advisory and the sender can terminate the
subscription prior to or after the expiry time.
