...
cached as part of the Track or Object and MUST be forwarded by relays, except
for Mandatory Track Extensions as described in {{mandatory-track-extensions}}.
If a Track or Object arrives with a different set of unknown extensions than
...
### Mandatory Track Extensions {#mandatory-track-extensions}

Extension Header types in the range 0x40-0x7F are designated as Mandatory Track
Extensions. These extensions have special handling rules that differ from other
Extension Headers.

When a relay receives Track Extensions (in PUBLISH, SUBSCRIBE_OK, or FETCH_OK
messages) containing a Mandatory Track Extension type that it does not
...
1. For PUBLISH messages: respond with PUBLISH_DONE containing an appropriate
   error code indicating the extension is not supported.
...
This behavior ensures that publishers can rely on mandatory extensions being
understood by all relays in the forwarding path. If negotiation of extension
support is not possible end-to-end, this range provides a fail-safe mechanism
where unknown mandatory extensions cause tracks to not be forwarded rather than
being forwarded incorrectly.

Mandatory Track Extensions only apply to Track scope extensions. Object scope
extensions do not have a mandatory range. Implementations MUST NOT register
types in the range 0x40-0x7F for Object Extensions.
...
* Message types
* MOQ Extension headers - Intent is to reserve extension types 0x00-0x3F (0-63)
  for standards utilization where space is a premium, 0x40-0x7F (64-127) for
  Mandatory Track Extensions (see {{mandatory-track-extensions}}), 0x80-0x3FFF
  ...
  usage. List which headers can be repeated in the table.
...
Types in the range 0x40-0x7F are reserved for Mandatory Track Extensions
(see {{mandatory-track-extensions}}). Extensions registered in this range
MUST have Track scope; Object scope extensions MUST NOT be registered in
this range.
